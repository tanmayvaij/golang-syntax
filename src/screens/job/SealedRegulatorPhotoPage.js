...
export default function SealedRegulatorPhotoPage() {
  ...
  console.log("SealedRegulatorPhotoPage");
  const [selectedImage, setSelectedImage] = useState();

  useEffect(() => {
    const regulatorPhoto =
      streamImages != null
        ? Object.entries(streamImages).find(
            ([key]) => key === `regulatorPhoto_${count}`
          )
        : null;
    const photo = regulatorPhoto ? regulatorPhoto[1] : null;
  ...
  const backPressed = () => {
    streamImages = {
      ...streamImages,
      [`regulatorPhoto_${count}`]: selectedImage,
    };
  ...
  const nextPressed = async () => {
    ...
    navigation.navigate("VentOutKioskPhotoPage");
  };
    ...
    <ScrollView style={styles.scrollView}>
      <SafeAreaView style={styles.content}>
        <Header
          ...
          hasCenterText={true}
          hasRightBtn={true}
          centerText={"Photo of Sealed Regulator"}
