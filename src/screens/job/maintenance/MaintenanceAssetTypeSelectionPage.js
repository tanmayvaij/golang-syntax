...
import {height, unitH, unitW, width} from '../../../utils/constant';
...
function AssetTypeSelectionPage() {
  ...
  const [mprn, setMprn] = useState(meterDetails?.mprn);
  ...
  const nextPressed = () => {
    ...
    if (!isMeter && !isAmr && !isCorrector && !isDataLogger) {
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView style={styles.content}>
          <View style={styles.body}>
            ...
            <View style={styles.mprn}>
              <Text type={TextType.CAPTION_2}>MPRN:</Text>
              <TextInput
                value={mprn}
                onChangeText={txt => {
            ...
            <View style={styles.spacer} />
            {/* <SwitchWithTitle
              title={'Pulse Splitter'}
              value={isPulseSplitter}
              onValueChange={e => {
                setIsPulseSplitter(e);
              }}
            /> */}
            <SwitchWithTitle
              title={'Data Logger'}
