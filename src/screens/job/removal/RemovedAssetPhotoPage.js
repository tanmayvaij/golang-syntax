...
export default function RemovedAssetPhotoPage() {
  ...
  const [selectedImage, setSelectedImage] = useState(
    removedMeterDetails?.assetPhoto
  );

  console.log("RemovedAssetPhotoPage");

  const backPressed = () => {
    if (jobType === "Removal" || jobType === "Warrant") {
      appContext.setRemovedMeterDetails({
        ...removedMeterDetails,
        assetPhoto: selectedImage,
      });
    } else if (jobType === "Exchange") {
      // start install process
      //appContext.setPassedRemoval(true);
    }
    navigation.goBack();
  };
