...
export default function RegulatorPhotoPage() {
  ...
  const [selectedImage, setSelectedImage] = useState(
    regulatorDetails?.regulatorPhoto
  );
  console.log("RegulatorPhotoPage");
  console.log(">>...", regulatorDetails?.regulatorPhoto);

  const backPressed = () => {
    appContext.setRegulatorDetails({
      ...regulatorDetails,
      regulatorPhoto: selectedImage,
    });
    navigation.goBack();
  };
  ...
  const nextPressed = async () => {
    ...
    if (regulatorDetails.isChatBox === true) {
      navigation.navigate("ChatterBoxPage");
    } else if (regulatorDetails.isAdditionalMaterial === true) {
      navigation.navigate("AdditionalMaterialPage");
    } else {
    ...
    <ScrollView style={styles.scrollView}>
      <SafeAreaView style={styles.content}>
        ...
        <View style={styles.body}>
          <Text type={TextType.BODY_1}>Picture of Regulator</Text>
          {selectedImage && (
            <Image
