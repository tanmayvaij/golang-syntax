...
export default function CompositeLabelPage() {
  ...
  const [selectedImage, setSelectedImage] = useState(
    standardDetails?.compositeLabelPhoto
  );

  console.log("CompositeLabelPage");

  const backPressed = () => {
    appContext.setStandardDetails({
      ...standardDetails,
      compositeLabelPhoto: selectedImage,
    });
    navigation.goBack();
  };
  ...
  const nextPressed = async () => {
    ...
    navigation.navigate("DsearLabelPhotoPage");
  };
      ...
      <SafeAreaView style={styles.content}>
        ...
        <View style={styles.body}>
          <Text type={TextType.BODY_1} style={{ textAlign: "left" }}>
            {`photo of composite label showing MPRN and pressure settings`}
          </Text>
          {selectedImage && (
            <Image
